...
class PDFPlanGenerationService:
    ...
    async def generate_event_plan_pdf(self, event_data: Dict, user_selections: Dict = None) -> bytes:
        ...
        print(f"🔍 PDF: Starting generation with event_data keys: {list(event_data.keys()) if event_data else 'None'}")
        print(f"🔍 PDF: User selections keys: {list(user_selections.keys()) if user_selections else 'None'}")
        ...
        try:
            ...
            title = f"{str(event_type).title()} Plan"
            print(f"🔍 PDF: Creating title: {title}")
            story.append(Paragraph(title, self.styles['EventTitle']))
            story.append(Spacer(1, 20))
            
            # Event Overview
            print(f"🔍 PDF: Creating event overview")
            story.extend(self._create_event_overview(event_data))
            
            # Generate AI-powered comprehensive plan
            print(f"🔍 PDF: Generating AI plan")
            ai_plan = await self._generate_ai_plan(event_data, user_selections)
            
            # Add AI-generated sections
            print(f"🔍 PDF: Adding AI sections")
            story.extend(self._create_ai_generated_sections(ai_plan))
            
            # Footer
            print(f"🔍 PDF: Adding footer")
            story.append(Spacer(1, 20))
            footer_text = f"Generated by Tiles AI Event Planner on {datetime.now().strftime('%B %d, %Y')}"
            story.append(Paragraph(footer_text, self.styles['Normal']))
            
            # Build PDF
            print(f"🔍 PDF: Building document with {len(story)} story elements")
            doc.build(story)
    ...
    def _create_event_overview(self, event_data: Dict) -> List:
        ...
        # Event details table
        details = [
            ["Event Type", str(event_data.get('event_type') or 'Not specified')],
            ["Location", str(event_data.get('location') or 'Not specified')],
            ["Guest Count", str(event_data.get('guest_count') or 'Not specified')],
            ["Budget", str(event_data.get('budget') or 'Not specified')],
            ["Meal Type", str(event_data.get('meal_type') or 'Not specified')],
            ["Dietary Restrictions", str(event_data.get('dietary_restrictions') or 'None')]
        ]
...
- Be professional, detailed, and actionable"""
    ...
    def _create_ai_generated_sections(self, ai_plan: Dict) -> List:
        ...
        content = []
        print(f"🔍 PDF: Creating AI sections with plan keys: {list(ai_plan.keys()) if ai_plan else 'None'}")
        
        # Executive Summary
        if ai_plan.get('executive_summary'):
            print(f"🔍 PDF: Processing executive_summary: {type(ai_plan.get('executive_summary'))}")
            content.append(Paragraph("Executive Summary", self.styles['SectionHeader']))
            summary_value = ai_plan.get('executive_summary', '')
            summary_text = str(summary_value) if summary_value is not None else "No executive summary available"
            print(f"🔍 PDF: Summary text length: {len(summary_text)}")
            content.append(Paragraph(summary_text, self.styles['Normal']))
            content.append(Spacer(1, 15))
        
        # Timeline
        if ai_plan.get('timeline'):
            print(f"🔍 PDF: Processing timeline: {type(ai_plan.get('timeline'))}")
            content.append(Paragraph("📅 Detailed Timeline", self.styles['SectionHeader']))
            timeline_items = ai_plan.get('timeline', [])
            if isinstance(timeline_items, list):
                print(f"🔍 PDF: Timeline has {len(timeline_items)} items")
                for i, item in enumerate(timeline_items):
                    print(f"🔍 PDF: Timeline item {i}: {type(item)} - {item}")
                    item_text = str(item) if item is not None else "Timeline item"
                    content.append(Paragraph(f"• {item_text}", self.styles['RecommendationItem']))
            else:
                print(f"🔍 PDF: Timeline not a list, using as text")
                timeline_text = str(timeline_items) if timeline_items is not None else "No timeline available"
                content.append(Paragraph(timeline_text, self.styles['Normal']))
            content.append(Spacer(1, 15))
        
        # Budget Breakdown
        if ai_plan.get('budget_breakdown'):
            print(f"🔍 PDF: Processing budget_breakdown: {type(ai_plan.get('budget_breakdown'))}")
            content.append(Paragraph("💰 Budget Breakdown", self.styles['SectionHeader']))
            budget_data = ai_plan.get('budget_breakdown', {})
            
            if isinstance(budget_data, dict):
                print(f"🔍 PDF: Budget dict has {len(budget_data)} items")
                # Format as structured budget items
                for category, cost in budget_data.items():
                    print(f"🔍 PDF: Budget item - {category}: {cost} (types: {type(category)}, {type(cost)})")
                    category_text = str(category).title() if category is not None else "Budget item"
                    cost_text = str(cost) if cost is not None else "TBD"
                    content.append(Paragraph(f"• {category_text}: {cost_text}", self.styles['RecommendationItem']))
            else:
                print(f"🔍 PDF: Budget not a dict, using as text")
                # Fallback to text
                budget_text = str(budget_data) if budget_data is not None else "No budget breakdown available"
        ...
        if ai_plan.get('vendor_coordination'):
            ...
            vendor_value = ai_plan.get('vendor_coordination', '')
            vendor_text = str(vendor_value) if vendor_value is not None else "No vendor coordination details available"
        ...
        if ai_plan.get('guest_experience'):
            ...
            guest_value = ai_plan.get('guest_experience', '')
            guest_text = str(guest_value) if guest_value is not None else "No guest experience details available"
        ...
        if ai_plan.get('logistics'):
            ...
            logistics_value = ai_plan.get('logistics', '')
            logistics_text = str(logistics_value) if logistics_value is not None else "No logistics details available"
        ...
        if ai_plan.get('final_checklist'):
            ...
            checklist_items = ai_plan.get('final_checklist', [])
            if isinstance(checklist_items, list):
                for item in checklist_items:
                    item_text = str(item) if item is not None else "Checklist item"
                    content.append(Paragraph(f"☐ {item_text}", self.styles['RecommendationItem']))
            else:
                checklist_text = str(checklist_items) if checklist_items is not None else "No checklist available"
